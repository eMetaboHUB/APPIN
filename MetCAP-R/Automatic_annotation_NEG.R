...
## This script is a tool to automatize the annotation propagation of cured data matched with internal database in NEGATIVE ion mode.
...
## - Specific annotation on chlores adduct signals
...
data_corrected_motif <- remove_motif_isotope_NEG(data)
...
data_cleaned_filled <- data_corrected_attribution %>%
  mutate(
    MSMS = ifelse(Verify_motif_NEG(data_corrected_attribution), "CID", MSMS),
    lvl = ifelse(Verify_motif_NEG(data_corrected_attribution), "2", lvl),
    annotation_confidence = ifelse(Verify_motif_NEG(data_corrected_attribution), "a,b,c", annotation_confidence)
  )

#################################### Fill the annotation columns for lines with [M] isotopes and [M-H]- attribution
data_cleaned_filled <- data_cleaned_filled %>%
  mutate(
    MSMS = ifelse(verify_isotope_NEG(data_cleaned_filled) & Verify_motif_NEG(data_cleaned_filled), "CID", MSMS),
    lvl = ifelse(verify_isotope_NEG(data_cleaned_filled) & Verify_motif_NEG(data_cleaned_filled), "2", lvl),
    annotation_confidence = ifelse(verify_isotope_NEG(data_cleaned_filled) & Verify_motif_NEG(data_cleaned_filled), "a,b,c", annotation_confidence)
  )
...
data_inf_2.5 <- Filter_attribution_NEG(data_inf_2.5)
...
#################################### Annotate with CIDm/z-35 if already annotate and attribution is [M+Cl]- 
data_annoted <- data_annoted %>%
  mutate(
    MSMS = ifelse(Verify_motif_Cl(data_annoted), "CIDm/z-35", MSMS))
