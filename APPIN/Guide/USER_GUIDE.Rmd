...
[![License: CeCILL 2.1](https://img.shields.io/badge/License-CeCILL%202.1-blue.svg)](https://cecill.info/licences/Licence_CeCILL_V2.1-en.html)

**APPIN** (Automated Peak Picking and INtegration) is a Shiny application designed for loading, visualizing, processing, and exporting 2D NMR spectra from Bruker instruments. It provides a complete interactive interface optimized for metabolomics research, from raw Bruker files to publication-ready intensity tables.
...
## Step 2: Plot Settings {#plot-settings}
...
> **Important:** Always verify the spectrum type matches your actual experiment before generating the plot. Wrong defaults will give unusable results.
...
> **HSQC note:** The `% of max` method can give inconsistent thresholds for HSQC due to varying ¹³C intensities. The app displays a warning suggesting `Noise ×` instead.

Click **"Generate Plot"** to display the spectrum.

---

## Step 3: Peak Picking {#peak-picking}
...
> **Important:** Running Local Max after CNN (or vice versa) overwrites the previous results. There is no automatic merging between methods.
...
**Options (in Options collapsible):**
...
> **Note:** Fusion modifies your data **immediately** — not via the pending workflow. The fused peak is logged in the "Pending Fusions" table for traceability only. Clicking **Discard** at the bottom does not undo a fusion.

### Deleting Peaks and Boxes {#delete}
...
> **Same as Fusion:** Deletion is applied immediately. The deleted items are logged in "Pending Deletions" for traceability only. Cannot be undone via Discard.
...
> **Validation:** APPIN rejects boxes where xmin ≥ xmax, ymin ≥ ymax, or any side < 0.001 ppm. An error notification will appear.

### Add Manually {#manual-add}
...
## Step 5: Pending Changes — Apply or Discard {#pending-changes}
...
## Step 6: Integration {#integration}
...
### R² Fallback Behavior {#r2-fallback}
...
Click **"Run Integration"** to process all boxes. The summary appears below the button, and the **Fit Quality** tab fills with diagnostics.

> **Negative intensities:** Some peaks may show negative volumes. This can be expected (CH₂ groups in multiplicity-edited HSQC have inverted phase) or indicate phase / baseline issues. APPIN warns you when this happens. Exports replace negatives with 0 by default.

---

## Step 7: Fit Quality (Read-only Diagnostics) {#fit-quality}
...
## Step 8: Save & Export {#save-export}
...
### Dynamic Shift Recentering (Batch Export) {#shift-recentering}
...
# ID Naming Conventions {#id-conventions}
